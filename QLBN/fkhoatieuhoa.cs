...
namespace QLBN
{
    public partial class fkhoatieuhoa : Form
    {
        private ListBenhNhan ListBenhNhan;
        public fkhoatieuhoa()
        {
            InitializeComponent();
            LoadBenhNhanKhoaTieuHoa();
            LoadGB();
        ...
        private void dtgvkhoatieuhoa_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ListBenhNhan.LoadDataFromSelectedRow(dtgvkhoatieuhoa, txbid, txbhoten, cbGT, DtNgaySinh, txbCCCD, txbBHYT, txbSDT, NgayNhapVien, NgayRaVien, txbkhoa, txbBacsi, txbYta, txbBenh, cbGB, cbLBN);
        }
        void LoadBenhNhanKhoaTieuHoa()
        {
            string query = "SELECT * FROM dbo.Benhnhan Where MaKhoa=13";
            DataProvider provider = new DataProvider();
            dtgvkhoatieuhoa.DataSource = provider.ExecuteQuery(query);
        }
        ...
        {
            ListBenhNhan.TimKiemBN(dtgvkhoatieuhoa, txbTK, "13");
        }
